...
function check () {
    ...
    if dpkg --list python3-pip; then CHECKS=$((CHECKS+=1)); sudo python3 -m pip install -r requirements.txt; fi;
...
if [ "$INSTALL" -eq 1 ]; then
    ...
    if dpkg --list python3-pip; then echo "python3-pip installed"; sudo python3 -m pip install -r requirements.txt;; else sudo apt-get install python3-pip -y;sudo python3 -m pip install -r requirements.txt; fi
    check
else
    check
fi
